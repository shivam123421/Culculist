...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize history button and back button
    ...
    function initializeAdMob() {
        try {
            const config = window.ADMOB_CONFIG;
            if (!config || !config.appId || !config.bannerAdUnit) {
                console.warn('AdMob configuration is missing or incomplete');
                return;
            }
            ...
            adElement.setAttribute('data-ad-client', `ca-pub-${config.appId}`);
            adElement.setAttribute('data-ad-slot', config.bannerAdUnit);
